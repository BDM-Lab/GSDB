	...
	then
		...
		for directory in $rootdir; do
				...
				then			
					...
					chr_path+="/chr*_miniMDS.bed"
					genome_file="${directory}/${rootname}_All_Genome_miniMDS.bed"
					...
					do				
						...
						{	
							...
							echo "#!/bin/sh"
							echo "#!/bin/bash -l"
							echo "#SBATCH -J miniMDS_run"
							echo "#SBATCH -o miniMDS_run-%j.out"
							echo "#SBATCH -p Lewis"
							echo "#SBATCH -N 1"
							echo "#SBATCH -n 1"
							echo "#SBATCH --mem 80G"
							...
							echo "matlab_name=\"call_MDS_\${rand}.m\""
							echo "call_script=\"/storage/htc/bdm/tosin/GSDB_Scripts/Algorithms/miniMDS/Access/\${matlab_name}\";"
							
							echo " #Call the miniMDS alg_parameters	"
							echo "NAME=\"`basename "$local_file" .bed`\""							
							echo "outname=\"${alg_dir}/\${NAME}.tsv\""
							echo "python  /storage/htc/bdm/tosin/GSDB_Scripts/Algorithms/miniMDS/minimds.py -m 0.01 -p 0.01 -o \$outname  $local_file"   
													
												
							echo "#Call Assessment code here"
							echo "Name=\"`basename "$local_file" _miniMDS.bed`\""	
							echo "listName=\"${directory}/\${Name}_list.txt\""
							echo "matrixName=\"${directory}/\${Name}_matrix.txt\""		
							
							echo "Res=\`sed -n '2{p;q}' \${outname}\`"
							echo "echo \"Resolution = \${Res}\""
							...
							echo "echo \"filename='\${outname}';\""
							echo "echo \"fname='\${listName}';\""
							echo "echo \"filenm='\${matrixName}';\""
							echo "echo \"outname='\${newfname}';\""
							echo "echo \"logout='\${outlog}';\"" 
							echo "echo \"Res=\${Res};\""
							echo "echo \"extract_pdb_scc\""
					...
					NAME=`basename "${genome_file}" .bed`
					outname="${genomepath}/${NAME}.tsv"
					python  /storage/htc/bdm/tosin/GSDB_Scripts/Algorithms/miniMDS/minimds.py -m 0.01 -p 0.01 -o $outname  $genome_file   
