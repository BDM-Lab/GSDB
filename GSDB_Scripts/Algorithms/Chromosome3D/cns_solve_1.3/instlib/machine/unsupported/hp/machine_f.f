...
C
C +++++++++++++++++++++++++++++++++++++++
C machine dependent NeXT/UNIX version
C +++++++++++++++++++++++++++++++++++++++
C
      IMPLICIT NONE
C input/output
...
C
C +++++++++++++++++++++++++++++++++++++++
C machine dependent CRAY-2/UNICOS version
C +++++++++++++++++++++++++++++++++++++++
C
...
C input/output
      CHARACTER*(*) USERNM
      INTEGER MAX, LENGTH, LEN
C local
      CHARACTER*5  USER0
C begin
      USER0='USER'
      USER0=USER0(1:4)//CHAR(0)
      CALL GETNV(USER0,USERNM,LEN)
      USERNM(LEN+1:MAX)=' '
      LENGTH=MAX
...
C it in USERNM.
C +++++++++++++++++++++++++++++++++++++++
C machine dependent STELLAR/UNIX version
C +++++++++++++++++++++++++++++++++++++++
C
      IMPLICIT NONE
C input/output
      ...
      INTEGER MAX, LENGTH
C begin
      CALL COPYST(USERNM,MAX,LENGTH,'HP/HP-UX',8)
      RETURN
      END
C=====================================================================
...
C =======================
C modified (create "version numbers" on STELLAR): Michael Nilges 21-MAY-87
C Argument passing to c routine 'rename'; Tom Simonson, Strasbourg, Jan. 1993.
C
      IMPLICIT NONE
C input/output    
      INCLUDE 'iochan.inc'
...
CC     &     UNIT=6)
C
C call the routine that updates the disk after each write. 
        CALL OUTBUF
C
...
C write access
      ...
      CALL RENAME(%REF(LSTFIL(1:FILLEN)//CHAR(0)),
     &            %REF(LSTFIL(1:FILLEN)//';1'//CHAR(0))) 
      DO WHILE (EXISTS)
      CALL NEXTVERSION(LSTFIL)
      FILEN1=LEN(LSTFIL)
      CALL TRIMM(LSTFIL,FILEN1)
      INQUIRE(FILE=LSTFIL,EXIST=EXISTS)
      IF (EXISTS) THEN
      CALL RENAME(%REF(LSTFIL(1:FILEN1)//CHAR(0)),
     &            %REF(LSTFIL(1:FILLEN)//';2'//CHAR(0)))
      CALL RENAME(%REF(LSTFIL(1:FILLEN)//';1'//CHAR(0)),
     &            %REF(LSTFIL(1:FILEN1)//CHAR(0)))
      CALL RENAME(%REF(LSTFIL(1:FILLEN)//';2'//CHAR(0)),
     &            %REF(LSTFIL(1:FILLEN)//';1'//CHAR(0)))
      END IF
      END DO
      FILEN1=LEN(LSTFIL)
...
C
C Creates "version numbers" on STELLAR if a "NEW" file
C  is already there.
...
C
      CALL GETNV(WD(1:LL-1)//CHAR(0),PATH,LPATH)
      IF (LPATH.GT.MXPATH) THEN
      CALL DSPERR('XXNXTFI', 'pathname too long')
      GOTIT=.FALSE.
      ELSE
      L=LPATH
      CALL TRIMM(PATH,L)
...
C temporary space.
C machine-dependent CONVEX routine
C
C Author: Axel T. Brunger
C
      IMPLICIT NONE
...
C
      ...
      INTEGER IUNIT, STLEN, LPATH
      CHARACTER*(COMMAX) LINE
      LOGICAL ERR, QEXIST
C begin
      CALL GETNV('CNS_HELPLIB'//CHAR(0),PATH,LPATH)
      IF (LPATH.GT.MXPATH) THEN
      CALL DSPERR('CNS-HELP', 'pathname too long')
      RETURN
      ELSE
      STLEN=LPATH
