...
C
C +++++++++++++++++++++++++++++++++++++++
C machine dependent DEC alpha/OSF1 version
C +++++++++++++++++++++++++++++++++++++++
C
      IMPLICIT NONE
C input/output
...
C input/output
      ...
      INTEGER MAX, LENGTH
C begin
      CALL COPYST(USERNM,MAX,LENGTH,'DEC/DEC-Unix',12)
      RETURN
      END
C=====================================================================
...
C
C Creates "version numbers" on DEC Alpha if a "NEW" file
C  is already there.
...
C
C +++++++++++++++++++++++++++++++++++++++
C machine dependent DEC Alpha/UNIX version
C +++++++++++++++++++++++++++++++++++++++
C
C Axel Brunger, 30-APR-85
C =======================
C modified (create "version numbers" on sgi): Michael Nilges 21-MAY-87
C
      IMPLICIT NONE
C input/output    
      INCLUDE 'iochan.inc'
...
C write access
      ...
      FILLEN=LEN(FILE)
      CALL TRIMM(FILE,FILLEN)
      IF (WRNLEV.GE.15) WRITE(6,'(A)') ' -> rename'
      CALL RENAME(LSTFIL(1:FILLEN),LSTFIL(1:FILLEN)//';1') 
      DO WHILE (EXISTS)
      CALL NEXTVERSION(LSTFIL)
      INQUIRE(FILE=LSTFIL,EXIST=EXISTS)
      IF (EXISTS) THEN
      IF (WRNLEV.GE.15) WRITE(6,'(A)') ' -> 2nd rename'
      CALL RENAME(LSTFIL,LSTFIL(1:FILLEN)//';2')
      CALL RENAME(LSTFIL(1:FILLEN)//';1',LSTFIL)
      CALL RENAME(LSTFIL(1:FILLEN)//';2',LSTFIL(1:FILLEN)//';1')
      END IF
      END DO
      IF (WRNLEV.GE.15) WRITE(6,'(A)') ' -> 3rd rename'
      CALL RENAME(LSTFIL(1:FILLEN)//';1',LSTFIL)
      END IF
      IF (WRNLEV.GE.15.) WRITE(6,'(A)') ' -> open'
      IF (FORM.EQ.'FORMATTED') THEN
         OPEN(FILE=FILE,STATUS='NEW',ACCESS='SEQUENTIAL',
     &        ERR=9999,UNIT=UNIT,FORM='FORMATTED')
      ELSE
         OPEN(FILE=FILE,STATUS='NEW',ACCESS='SEQUENTIAL',
     &        ERR=9999,UNIT=UNIT,FORM='UNFORMATTED')
      ENDIF
C---------------------------------------------------------------
      ELSE
      WRITE(6,'(3A)')
...
C temporary space.
C machine-dependent DEC Alpha routine
C
C Author: Axel T. Brunger
C
      IMPLICIT NONE
