...
C input/output
      ...
      CHARACTER*100 NAME
      INTEGER LEN
      LOGICAL QOPEN, QFORM, QWRITE, ISATTY
C begin
      QTERM = ISATTY(STREAM)
      RETURN
      END
...
C input/output
      ...
      INTEGER MAX, LENGTH
C begin
      CALL COPYST(USERNM,MAX,LENGTH,'Convex',6)
      RETURN
      END
C=====================================================================
...
C
C Creates "version numbers" on CONVEX if a "NEW" file
C  is already there.
...
C =======================
C modified (create "version numbers" on CONVEX): Michael Nilges 21-MAY-87
C
      IMPLICIT NONE
C input/output    
      INCLUDE 'iochan.inc'
