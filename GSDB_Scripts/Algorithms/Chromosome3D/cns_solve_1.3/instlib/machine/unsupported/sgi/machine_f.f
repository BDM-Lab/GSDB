...
C input/output
      ...
      INTEGER MAX, LENGTH
C begin
      CALL COPYST(USERNM,MAX,LENGTH,'SGI/IRIX',8)
      RETURN
      END
C=====================================================================
...
C
C Creates "version numbers" on sgi if a "NEW" file
C  is already there.
...
C temporary space.
C machine-dependent sgi routine
C
C Author: Axel T. Brunger
C
      IMPLICIT NONE
...
C begin
      ...
      HNLEN = 7
C
      IF (GETHOSTNAME(LHN, LHNLEN) .EQ. 0) THEN
        HNLEN = MIN(LEN(HOSTNM), LHNLEN)
        DO I = 1, HNLEN
          IF (HNLEN .GE. I) THEN
            IF (LHN(I) .EQ. ' ' .OR. LHN(I) .EQ. CHAR(0)) THEN
              HNLEN = I - 1
              HOSTNM(I:I) = ' '
            ELSE
              HOSTNM(I:I) = LHN(I)
