...
C input/output                                                          
      ...
      INTEGER MAX, LENGTH                                               
C begin                                                                 
      CALL COPYST(USERNM,MAX,LENGTH,'IBM/AIX',7)
      RETURN                                                            
      END                                                               
C=====================================================================  
...
C =======================                                               
C modified (create "version numbers" on STELLAR): Michael Nilges 21-MAY-
C                                                                       
      IMPLICIT NONE                                                     
C input/output                                                          
      INCLUDE 'iochan.inc'                                              
...
CC     &     UNIT=6)                                                    
C                                                                       
C call the routine that updates the disc after each write.              
        call outbuf                                                     
C                                                                       
...
C                   *  this is automatic     *                          
C                   *   if the file is       *                          
C                   *  opened with type old  *                          
C                   *    under aix os        *                          
C                   **************************                          
C----------------------------------------------------------------       
      IF (ACCESS.EQ.'APPEND') THEN                                      
      OPEN(FILE=FILE,FORM=FORM,STATUS='OLD',ERR=9999,UNIT=UNIT)         
C---------------------------------------------------------------        
C read access                                                           
...
C write access                                                          
      ...
      FILLEN=LEN(FILE)                                                  
      CALL TRIMM(FILE,FILLEN)                                           
      CALL IBMMV(LSTFIL(1:FILLEN)//CHAR(0),
     &      LSTFIL(1:FILLEN)//';1'//CHAR(0)) 
      DO WHILE (EXISTS)
      CALL NEXTVERSION(LSTFIL)
      FILLE2=LEN(LSTFIL)                                                  
      CALL TRIMM(LSTFIL,FILLE2)                                           
      INQUIRE(FILE=LSTFIL,EXIST=EXISTS)
      IF (EXISTS) THEN
      CALL IBMMV(LSTFIL(1:FILLE2)//CHAR(0),
     &           LSTFIL(1:FILLEN)//';2'//CHAR(0))
      CALL IBMMV(LSTFIL(1:FILLEN)//';1'//CHAR(0),
     &           LSTFIL(1:FILLE2)//CHAR(0))
      CALL IBMMV(LSTFIL(1:FILLEN)//';2'//CHAR(0),
     &           LSTFIL(1:FILLEN)//';1'//CHAR(0))
      END IF
      END DO
      CALL IBMMV(LSTFIL(1:FILLEN)//';1'//CHAR(0),
