...
C +++++++++++++++++++++++++++++++++++++++
C machine dependent UNICOS/mk-T3E version
C +++++++++++++++++++++++++++++++++++++++
C
      IMPLICIT NONE
C input/output
...
C begin
      ...
      IF (A(1:2).EQ.'01') THEN
      MONTH='Jan'
      ELSE IF (A(1:2).EQ.'02') THEN
      MONTH='Feb'
      ELSE IF (A(1:2).EQ.'03') THEN
      MONTH='Mar'
      ELSE IF (A(1:2).EQ.'04') THEN
      MONTH='Apr'
      ELSE IF (A(1:2).EQ.'05') THEN
      MONTH='May'
      ELSE IF (A(1:2).EQ.'06') THEN
      MONTH='Jun'
      ELSE IF (A(1:2).EQ.'07') THEN
      MONTH='Jul'
      ELSE IF (A(1:2).EQ.'08') THEN
      MONTH='Aug'
      ELSE IF (A(1:2).EQ.'09') THEN
      MONTH='Sep'
      ELSE IF (A(1:2).EQ.'10') THEN
      MONTH='Oct'
      ELSE IF (A(1:2).EQ.'11') THEN
      MONTH='Nov'
      ELSE IF (A(1:2).EQ.'12') THEN
      MONTH='Dec'
      END IF
C
      A=DAY//'-'//MONTH//'-'//YEAR
...
C
C +++++++++++++++++++++++++++++++++++++++
C machine dependent CONVEX/UNIX version
C +++++++++++++++++++++++++++++++++++++++
C
...
C begin
      ...
      IF (NAME(1:5).EQ.'INPUT' .AND. FSATTY(0).EQ.1) THEN
      QTERM=.TRUE.
      ELSE
      QTERM=.FALSE.
      END IF
...
C +++++++++++++++++++++++++++++++++++++++
C
C Author: Chris Haydock and Axel Brunger
C
C input/output
      CHARACTER*(*) USERNM
      INTEGER MAX, LENGTH
...
C input/output
      ...
      INTEGER MAX, LENGTH
C begin
      CALL COPYST(USERNM,MAX,LENGTH,'CRAY/unicosmk',13)
C
      RETURN
      END
C====================================================================
...
C=====================================================================
      SUBROUTINE NXTVER(FILE)
C
C Creates "version numbers" on NeXT if a "NEW" file
C  is already there.
...
C local
      ...
      INTEGER I, ISTAT, FILLEN, FILN, IUN, FILCHG
C begin
      ERR=.FALSE.
...
CC      OPEN(FILE=FILE,FORM=FORM,STATUS='OLD',ACCESS='APPEND',ERR=9999,
CC     &     UNIT=UNIT)
...
C begin
      ...
      IF (UNIT.LE.0.OR.UNIT.GT.100) QOPEN=.FALSE.
...
C I/O
      ...
      LOGICAL ERR
C
      INTEGER LENGTH, MAXLEN
      PARAMETER (MAXLEN=80)
      CHARACTER*(MAXLEN) FILENM
C begin
      ERR=.FALSE.
...
C +++++++++++++++++++++++++++++++++++++++
C machine dependent UNICOS version
C +++++++++++++++++++++++++++++++++++++++
C
C Axel T. Brunger
C ===============
...
C temporary space.
C machine-dependent CRAY routine
C
C Author: Axel T. Brunger
C
      IMPLICIT NONE
...
C
C     getenv using a posix call
C
      LENNAM = LEN(NAME)
      IF(LENNAM.LE.0) CALL WRNDIE(-5,'GETENV',
     &        'Zero length variable given to GETENV')
C
...
C =========================================================================
C
      SUBROUTINE RENAME(FROM,TO)
C
      IMPLICIT NONE
C
      CHARACTER*(*) FROM,TO
C local
      INTEGER LENFROM,LENTO,IERROR
C
C     rename using a posix call
C
      LENFROM = 0
      LENTO = 0
      CALL PXFRENAME(FROM,LENFROM,TO,LENTO,IERROR)
      IF(IERROR.NE.0) CALL WRNDIE(-5,'RENAME',
     &        'Error during call to RENAME')
      RETURN
      END
C
C=====================================================================
...
C local
      ...
      INTEGER      LHNLEN
      INTEGER      THNLEN
      PARAMETER(LHNLEN = 256)
      CHARACTER*(LHNLEN)  LHN
C
C external
      INTEGER   GETHOST
      EXTERNAL  GETHOST
...
C
      THNLEN = GETHOST(LHN)
      IF (THNLEN .GT. 0) THEN
        HNLEN = MIN(LEN(HOSTNM), LHNLEN)
        DO I = 1, HNLEN
          IF (HNLEN .GE. I) THEN
            IF (LHN(I:I) .EQ. ' ' .OR. LHN(I:I) .EQ. CHAR(0)) THEN
              HNLEN = I - 1
              HOSTNM(I:I) = ' '
            ELSE
              HOSTNM(I:I) = LHN(I:I)
