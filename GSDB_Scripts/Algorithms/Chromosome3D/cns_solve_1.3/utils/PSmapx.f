...
C---- 4/15/91 SRH
C---- Modified to plot sections along x where z is the intermediate
C---- and y is the fasted. 8/18/93 HW

      DIMENSION CL(50),IORGX(3),INCX(3),NG(3),
     $     NP(3),ZAPP(12),IALPHA(3)
C---- CHANGE THE DIMENSION OF rhoold TO SUIT THE SIZE OF YOUR MAP
      real*8 rho(400,400),cellDP(6),rhoold(1,400,400)
...
C END MODIFICATION ATB
      ...
      ng(2) = nzPts
      ng(3) = nyPts
      np(1) = nxEnd - nxStart + 1
      np(2) = nzEnd - nzStart + 1
      np(3) = nyEnd - nyStart + 1
      isum1 = 3
      isum2 = 2
      isum3 = 1
      ialpha(1) = 'x'
...
C MODIFICATION 7/12/1006 ATB
      do iz=1,np(2)
         read(10,'(I8)')
         read(10,'(6E12.5)') ((rhoold(ix,iy,iz),ix=1,np(1)),iy=1,np(3))
      enddo
C END MODIFICATION
C 
C---- PLOT SECTIONS
