...
C---- 4/15/91 SRH
C---- Modified to plot sections along y where x is the intermediate
C---- and z is the fastest. 8/18/93 HW

      DIMENSION CL(50),IORGX(3),INCX(3),NG(3),
     $     NP(3),ZAPP(12),IALPHA(3)
C---- CHANGE THE DIMENSION OF rhoold TO SUIT THE SIZE OF YOUR MAP
      real*8 rho(400,400),cellDP(6),rhoold(400,1,400)
...
C END MODIFICATION ATB
      ...
      ng(2) = nxPts
      ng(3) = nzPts
      np(1) = nyEnd - nyStart + 1
      np(2) = nxEnd - nxStart + 1
      np(3) = nzEnd - nzStart + 1
      isum1 = 3
      isum2 = 2
      isum3 = 1
      ialpha(1) = 'y'
...
C     DO 10 I = 1,M
C     10	READ (10)
C     read(10)			! map section number
C     10    	read(10)			! 2-D map section
C     
C---- READ THE WHOLE MAP INTO MEMORY
      do iz=1,np(3)
         read(10,'(I8)')
         read(10,'(6E12.5)') ((rhoold(ix,iy,iz),ix=1,np(2)),iy=1,np(1))
      enddo
C---- PLOT SECTIONS
      XOFF=XO
...
C     READ(10) IXX1,IXX2,IXX3,LW,(RHO(I,J),J=1,LW)
              ...
              do ix=1,np(2)
                 do iz=1,np(3)
                    rho(ix,iz)=rhoold(ix,iy,iz)
                 enddo
              enddo
C     read(10) ixx1
C     read(10) ((rho(i,j), j=1,lw), i=1,m)
              ixx2 = 0
              ixx3 = 0
C     10000	CONTINUE
...
C     CALL NEWPEN (LINWID)
     ...
     @                1.0+xoff, ' in ', 1.0, ' in translate'
                 ...
                 XZR =  0.00
                 YZR = -0.40
                 IF (ITICK.EQ.1) CALL PSTRNG (LABELX, LBX,XZR,YZR,HGH)
                 XZR = -0.25
                 YZR =  0.00
                 IF (ITICK.EQ.1) CALL PSTRNG (LABELY,-LBX,XZR,YZR,HGH)
                 write(25, '(a,/,a,/a)') 
